...
final class StreamConcatenation {
   ...
   *
   * <p>Although this method does not eagerly consume the elements of the input streams, this method
   * is a terminal operation for the input streams.
   *
   * <p>The returned stream is parallel if any of the input streams is parallel.
   *
   * <p>When the returned stream is closed, the close handlers for all the input streams are
   * invoked. If one of those handlers throws an exception, that exception will be rethrown after
   * the remaining handlers are invoked. If the remaining handlers throw exceptions, those
   * exceptions are added as suppressed exceptions of the first.
   *
   * <p>If the argument array or any of the input streams are modified after being passed to this
   ...
   * @throws NullPointerException if the argument array or any of the input streams are {@code null}
   */
  @SafeVarargs
