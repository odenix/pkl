...
abstract class MultivalueArbitraryBase<T, U> extends TypedCloneable
  ...
  public StreamableArbitrary<T, U> ofMaxSize(int maxSize) {
    ...
    if (maxSize < minSize) {
      String message =
          String.format("minSize (%s) must not be larger than maxSize (%s)", minSize, maxSize);
      throw new IllegalArgumentException(message);
    }
  ...
  protected RandomGenerator<List<T>> createListGenerator(
      int genSize, boolean withEmbeddedEdgeCases) {
  ...
  protected RandomGenerator<T> elementGenerator(
      Arbitrary<T> elementArbitrary, int genSize, boolean withEdgeCases) {
    return elementArbitrary.generator(genSize, withEdgeCases);
  }

  protected <C extends Collection<?>> EdgeCases<C> edgeCases(
      BiFunction<List<Shrinkable<T>>, Integer, Shrinkable<C>> shrinkableCreator, int maxEdgeCases) {
    // Optimization. Already handled by EdgeCases.concat(..)
    if (maxEdgeCases <= 0) {
      return EdgeCases.none();
    }

    EdgeCases<C> emptyListEdgeCase =
        (minSize == 0) ? emptyListEdgeCase(shrinkableCreator) : EdgeCases.none();

    int effectiveMaxEdgeCases = maxEdgeCases - emptyListEdgeCase.size();
    EdgeCases<C> singleElementEdgeCases =
        (minSize <= 1 && maxSize() >= 1)
    ...
    EdgeCases<C> fixedSizeEdgeCases =
        generateFixedSizeEdgeCases()
            ? fixedSizeEdgeCases(minSize, shrinkableCreator, effectiveMaxEdgeCases)
            : EdgeCases.none();

    return EdgeCasesSupport.concat(
        asList(emptyListEdgeCase, singleElementEdgeCases, fixedSizeEdgeCases), maxEdgeCases);
  }
  ...
  private <C extends Collection<?>> EdgeCases<C> fixedSizeEdgeCases(
    ...
    return EdgeCasesSupport.mapShrinkable(
        ...
        shrinkableT -> {
          List<Shrinkable<T>> elements =
              new ArrayList<>(Collections.nCopies(fixedSize, shrinkableT));
  ...
  private <C extends Collection<?>> EdgeCases<C> emptyListEdgeCase(
    ...
    return EdgeCases.fromSupplier(() -> shrinkableCreator.apply(Collections.emptyList(), minSize));
  }
