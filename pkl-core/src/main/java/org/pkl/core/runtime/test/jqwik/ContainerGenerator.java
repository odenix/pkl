...
import static org.pkl.core.runtime.test.jqwik.UniquenessChecker.checkValueUniqueIn;
...
class ContainerGenerator<T, C> implements RandomGenerator<C> {
  ...
  public Shrinkable<C> next(Random random) {
    ...
    boolean canUseSetForValues =
        uniquenessExtractors.isEmpty()
            || uniquenessExtractors.contains(FeatureExtractor.identity());
    Collection<T> existingValues = canUseSetForValues ? new HashSet<>() : new ArrayList<>();

    while (listOfShrinkables.size() < listSize) {
      try {
        Shrinkable<T> next =
            nextUntilAccepted(random, existingValues, elementGenerator::next, noDuplicates);
        ...
        if (noDuplicates) {
          // This should occur only rarely because usually the check against maxUniqueElements
    ...
    if (listOfShrinkables.size() > sizeToShuffleIfExceeded) {
      // If we started generating with no duplicates, and then realized we can't generate enough
      // unique elements,
      // then the list becomes skewed: unique elements go first
      // We shuffle the list to allow other constellations (e.g. list unique-most elements starting
      // with non-unique ones)
  ...
  private Shrinkable<T> nextUntilAccepted(
    ...
    String message =
        String.format(
            "Trying to fulfill uniqueness constraint missed more than %s times.", maxAttempts);
