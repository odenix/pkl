...
import static org.pkl.core.runtime.test.jqwik.UniquenessChecker.checkShrinkableUniqueIn;
...
abstract class ShrinkableContainer<C, E> implements Shrinkable<C> {
  ...
  @Nullable protected final Arbitrary<E> elementArbitrary;
  ...
  public Stream<Shrinkable<C>> shrink() {
    return JqwikStreamSupport.concat(
        shrinkSizeOfList(), shrinkElementsOneAfterTheOther(0), shrinkPairsOfElements());
  }
  ...
  private Stream<Shrinkable<C>> growOneElementAfterTheOther() {
    ...
    for (int i = 0; i < elements.size(); i++) {
      ...
      Stream<Shrinkable<C>> shrinkElement =
          element
              .grow()
              .flatMap(
                  shrunkElement -> {
          ...
          element
              ...
              .flatMap(
                  shrunkElement -> {
                    ...
                    if (!checkShrinkableUniqueIn(
                        uniquenessExtractors, shrunkElement, elementsCopy)) {
                      return Stream.empty();
                    }
  ...
  protected Stream<Shrinkable<C>> shrinkPairsOfElements() {
    ShrinkingCommons.ContainerCreator<C, E> createContainer =
        newElements -> {
  ...
  public String toString() {
    return String.format(
        "%s<%s>(%s:%s)",
        getClass().getSimpleName(), value().getClass().getSimpleName(), value(), distance());
  }
