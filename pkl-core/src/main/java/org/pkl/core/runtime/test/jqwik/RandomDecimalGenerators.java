...
public class RandomDecimalGenerators {
  ...
  public static RandomGenerator<BigDecimal> bigDecimals(
    ...
    RandomGenerator<BigInteger> unscaledBigIntegerGenerator =
        RandomIntegralGenerators.bigIntegers(
            genSize, unscaledRange.min, unscaledRange.max, unscaledShrinkingTarget, distribution);
  ...
  private static void checkRangeIsSound(Range<BigDecimal> range, int scale) {
    ...
    if (range.min.add(minimumDifference).compareTo(range.max) >= 0) {
      String message =
          String.format("No number with scale <%s> can be generated in %s", scale, range);
      throw new JqwikException(message);
    }
  }

  private static RandomGenerator<BigDecimal> scaledBigDecimalGenerator(
      final RandomGenerator<BigInteger> scaledBigIntegerGenerator, final int scale) {
    return scaledBigIntegerGenerator.map(value -> scaledBigDecimal(value, scale));
  }

  public static Range<BigInteger> unscaledBigIntegerRange(
      final Range<BigDecimal> range, final int scale) {
    BigInteger minScaled =
        range.minIncluded
            ? unscaledBigInteger(range.min, scale)
            : unscaledBigInteger(range.min, scale).add(BigInteger.ONE);
    BigInteger maxScaled =
        range.maxIncluded
  ...
  public static BigDecimal defaultShrinkingTarget(Range<BigDecimal> range, int scale) {
    if (range.includes(BigDecimal.ZERO)) return BigDecimal.ZERO;
    else {
