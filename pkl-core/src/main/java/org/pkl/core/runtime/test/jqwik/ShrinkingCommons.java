...
class ShrinkingCommons {
   ...
   */
  static <C, E> Stream<Shrinkable<C>> shrinkPairsOfElements(
      List<Shrinkable<E>> elements, ContainerCreator<C, E> createContainer) {
    return Combinatorics.distinctPairs(elements.size())
        .flatMap(
            pair ->
                JqwikStreamSupport.zip(
   ...
   * @return stream of shrunk containers
   */
  static <C, E> Stream<Shrinkable<C>> sortElements(
      List<Shrinkable<E>> elements, ContainerCreator<C, E> createContainer) {
    ...
    return JqwikStreamSupport.concat(
        fullSort(sortedElements, createContainer), pairwiseSort(elements, createContainer));
  }

  private static <C, E> Stream<Shrinkable<C>> fullSort(
      List<Shrinkable<E>> sortedElements, ContainerCreator<C, E> createContainer) {
    return Stream.of(createContainer.apply(sortedElements));
  }

  private static <C, E> Stream<Shrinkable<C>> pairwiseSort(
      List<Shrinkable<E>> elements, ContainerCreator<C, E> createContainer) {
    return Combinatorics.distinctPairs(elements.size())
        .map(
            pair -> {
