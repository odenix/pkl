...
public class FlatMappedShrinkable<T, U> implements Shrinkable<U> {
  ...
  public FlatMappedShrinkable(
      Shrinkable<T> toMap, Function<T, RandomGenerator<U>> toGeneratorMapper, long randomSeed) {
  ...
  public Stream<Shrinkable<U>> shrink() {
    return JqwikStreamSupport.concat(
        shrinkRightSide(), shrinkLeftSide(), shrinkLeftGrowRightSide());
  }
  ...
  public String toString() {
    return String.format(
        "%s<%s>(%s:%s)|%s",
        getClass().getSimpleName(), value().getClass().getSimpleName(), value(), distance(), toMap);
  }
}
