...
public class FilteredShrinkable<T> implements Shrinkable<T> {
  ...
  public Stream<Shrinkable<T>> shrink() {
    return Stream.concat(shrinkToFirst(toFilter), shrinkDeep(toFilter));
  }
  ...
  public Optional<Shrinkable<T>> grow(Shrinkable<?> before, Shrinkable<?> after) {
    if (before instanceof FilteredShrinkable && after instanceof FilteredShrinkable) {
      ...
      return toFilter
          .grow(beforeToFilter, afterToFilter)
          .filter(this::isIncluded)
          .map(this::toFiltered);
    }
    return toFilter.grow(before, after).filter(this::isIncluded).map(this::toFiltered);
  }
  ...
  private Stream<Shrinkable<T>> shrinkDeep(Shrinkable<T> base) {
    ...
    Stream<Shrinkable<T>> constrainedBaseShrink =
        base.shrink().takeWhile(ignore -> countBaseShrinks.get() < MAX_BASE_SHRINKS);
  ...
  public Stream<Shrinkable<T>> grow() {
    return Stream.concat(growToFirst(toFilter), growDeep(toFilter)).limit(50).distinct();
  }
  ...
  private Stream<Shrinkable<T>> growDeep(Shrinkable<T> base) {
    return Stream.concat(
            base.grow().flatMap(this::growToFirst), base.grow().flatMap(this::growDeep))
