...
public interface ArrayArbitrary<@Nullable T, A> extends StreamableArbitrary<T, A> {
  ...
  /**
   * Set random distribution {@code distribution} of size of generated array. The distribution's
   * center is the minimum size of the generated array.
   */
  ArrayArbitrary<T, A> withSizeDistribution(RandomDistribution uniform);

  /**
   * Add the constraint that elements of the generated array must be unique, i.e. no two elements
  ...
  /**
   * Add the constraint that elements of the generated array must be unique relating to an element's
