...
public class EdgeCasesSupport {
  ...
  public static <T> EdgeCases<T> concatFrom(
      final List<Arbitrary<T>> arbitraries, int maxEdgeCases) {
  ...
  public static <T, U> EdgeCases<U> mapShrinkable(
      EdgeCases<T> self, Function<Shrinkable<T>, Shrinkable<U>> mapper) {
  ...
  public static <T> EdgeCases<T> filter(EdgeCases<T> self, Predicate<T> filterPredicate) {
    List<Supplier<Shrinkable<T>>> filteredSuppliers =
        self.suppliers().stream()
            ...
            .map(
                supplier ->
                    (Supplier<Shrinkable<T>>)
                        () -> new FilteredShrinkable<>(supplier.get(), filterPredicate))
            .collect(Collectors.toList());
    return EdgeCases.fromSuppliers(filteredSuppliers);
  }

  public static <T> EdgeCases<T> ignoreExceptions(
      final EdgeCases<T> self, final Class<? extends Throwable>[] exceptionTypes) {
    List<Supplier<Shrinkable<T>>> filteredSuppliers =
        self.suppliers().stream()
            .filter(
                supplier -> {
  ...
  public static <T, U> EdgeCases<U> flatMapArbitrary(
      EdgeCases<T> self, Function<T, Arbitrary<U>> mapper, int maxEdgeCases) {
    List<Supplier<Shrinkable<U>>> flatMappedSuppliers =
        self.suppliers().stream()
            .flatMap(
                tSupplier -> {
                  T t = tSupplier.get().value();
                  return mapper.apply(t).edgeCases(maxEdgeCases).suppliers().stream()
                      .map(
                          uSupplier -> {
                            ...
                            return (Supplier<Shrinkable<U>>)
                                () ->
                                    new FixedValueFlatMappedShrinkable<>(
                                        tSupplier.get(), shrinkableMapper, uSupplier);
                          });
                })
  ...
  public static <T> EdgeCases<T> combine(
    ...
    for (Arbitrary<Object> a : arbitraries) {
      ...
      remainingEdgeCases =
          (int) Math.max(1, Math.ceil(remainingEdgeCases / (double) supplierList.size()));
    }
    ...
    while (iterator.hasNext() && count < maxEdgeCases) {
      ...
      List<Shrinkable<Object>> shrinkables =
          next.stream().map(Supplier::get).collect(Collectors.toList());
