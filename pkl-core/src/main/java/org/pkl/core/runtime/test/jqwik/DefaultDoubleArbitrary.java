...
public class DefaultDoubleArbitrary extends TypedCloneable implements DoubleArbitrary {
  ...
  public DefaultDoubleArbitrary() {
    this.generatingArbitrary =
        new DecimalGeneratingArbitrary(
            Range.of(toBigDecimal(DEFAULT_MIN), toBigDecimal(DEFAULT_MAX)));
  }
  ...
  public Arbitrary<Double> edgeCases(Consumer<EdgeCases.Config<Double>> configurator) {
    Consumer<EdgeCases.Config<BigDecimal>> decimalConfigurator =
        new MappedEdgeCasesConsumer<>(configurator, BigDecimal::doubleValue, BigDecimal::valueOf);
    DefaultDoubleArbitrary clone = typedClone();
    clone.generatingArbitrary =
        (DecimalGeneratingArbitrary) generatingArbitrary.edgeCases(decimalConfigurator);
    return clone;
  }
  ...
  public DoubleArbitrary between(double min, boolean minIncluded, double max, boolean maxIncluded) {
    DefaultDoubleArbitrary clone = typedClone();
    clone.generatingArbitrary.range =
        Range.of(toBigDecimal(min), minIncluded, toBigDecimal(max), maxIncluded);
    return clone;
  }
  ...
  public DoubleArbitrary greaterOrEqual(double min) {
    return between(
        min,
        true,
        generatingArbitrary.range.max.doubleValue(),
        generatingArbitrary.range.maxIncluded);
  }
  ...
  public DoubleArbitrary lessOrEqual(double max) {
    return between(
        generatingArbitrary.range.min.doubleValue(),
        generatingArbitrary.range.minIncluded,
        max,
        true);
  }
  ...
  private Arbitrary<Double> arbitrary() {
    ...
    return Arbitraries.frequencyOf(Tuple.of(49, doubleArbitrary), Tuple.of(1, specialsArbitrary));
  }
}
