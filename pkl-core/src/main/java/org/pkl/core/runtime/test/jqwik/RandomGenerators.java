...
public class RandomGenerators {
  ...
  public static RandomGenerator<BigInteger> bigIntegers(
      BigInteger min, BigInteger max, BigInteger shrinkingTarget, RandomDistribution distribution) {
  ...
  public static RandomGenerator<String> strings(
      ...
      long maxUniqueChars,
      int genSize,
      RandomDistribution lengthDistribution,
      Arbitrary<Character> characterArbitrary,
      boolean uniqueChars) {
    Function<List<Shrinkable<Character>>, Shrinkable<String>> createShrinkable =
        elements ->
            new ShrinkableString(elements, minLength, maxLength, characterArbitrary, uniqueChars);
    Set<FeatureExtractor<Character>> featureExtractors =
        uniqueChars ? ShrinkableString.UNIQUE_CHARS_EXTRACTOR : Collections.emptySet();
    return container(
        elementGenerator,
        createShrinkable,
        minLength,
  ...
  public static <T> RandomGenerator<List<T>> list(
    ...
    Function<List<Shrinkable<T>>, Shrinkable<List<T>>> createShrinkable =
        elements ->
            new ShrinkableList<>(
                elements, minSize, maxSize, uniquenessExtractors, elementArbitrary);
    return container(
        elementGenerator,
        createShrinkable,
        minSize,
  ...
  public static <T> RandomGenerator<T> samplesFromShrinkables(List<Shrinkable<T>> samples) {
    ...
    return ignored -> {
      if (tryCount.get() >= samples.size()) tryCount.set(0);
  ...
  public static <T> RandomGenerator<T> frequencyOf(
      List<Tuple2<Integer, Arbitrary<T>>> frequencies, int genSize, boolean withEmbeddedEdgeCases) {
    return frequency(frequencies).flatMap(Function.identity(), genSize, withEmbeddedEdgeCases);
  }

  public static <T> RandomGenerator<T> withEdgeCases(
